...
public class DelMoveDialog extends Stage {
    private final MoveDelUI delCommand;


    private final TextField id = new TextField();
    public DelMoveDialog(ServiceManager services, Stage owner) {
        this.delCommand = new MoveDelUI(services);

        this.initModality(Modality.WINDOW_MODAL); // Делает окно модальным
        this.initOwner(owner);
        this.setTitle("Удаление движения");
        setupLayout();
    }
